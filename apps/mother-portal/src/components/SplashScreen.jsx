...
export default function SplashScreen({ onDone }) {
  ...
  useEffect(() => {
    ...
    if (hour < 12) {
      setGreeting({
        en: "Good morning, Mama",
        sw: "Asubuhi njema, Mama"
      });
    } else if (hour < 16) {
      setGreeting({
        en: "Good afternoon, Mama",
        sw: "Mchana mwema, Mama"
      });
    } else {
      setGreeting({
        en: "Good evening, Mama",
        sw: "Habari ya jioni, Mama"
      });
    }

    const t1 = setTimeout(() => setPhase('show'), 100);
    const t2 = setTimeout(() => setPhase('exit'), 3800); // Allow time to admire the graphic
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-10 overflow-hidden">
        ...
        />
        <div 
          className="absolute top-[22%] right-[8%] w-20 h-20 rounded-full animate-float-2" 
          style={{
            background: 'radial-gradient(circle, rgba(253, 224, 71, 0.15) 0%, rgba(253, 224, 71, 0.04) 50%, transparent 80%)',
            filter: 'blur(10px)',
            mixBlendMode: 'screen',
            animationDelay: '4.8s'
          }} 
        ...
        />
        <div 
          className="absolute top-[28%] right-[22%] w-2 h-2 rounded-full animate-float-1" 
          style={{
            background: 'radial-gradient(circle, #FFFFFF 0%, rgba(253, 224, 71, 0.9) 60%, transparent 100%)',
      ...
      >
        <p 
          ...
          style={{ 
            ...
            color: '#10453A', // Rich forest green brand color
        ...
        <p 
          ...
          style={{ 
            fontFamily: '"Plus Jakarta Sans", sans-serif',
            color: '#C66C28', // Rich warm sunset orange
            mixBlendMode: 'multiply',
      ...
      <style>{`
        ...
        @keyframes float-spark-1 {
          ...
          20% { opacity: 0.65; }
          80% { opacity: 0.65; }
          100% { transform: translateY(-90px) translateX(25px) scale(1.3); opacity: 0; }
        }
        @keyframes float-spark-2 {
          0% { transform: translateY(0) translateX(0) scale(1.2); opacity: 0; }
          25% { opacity: 0.55; }
          75% { opacity: 0.55; }
          100% { transform: translateY(-120px) translateX(-20px) scale(0.7); opacity: 0; }
        }
        .animate-float-1 {
          animation: float-spark-1 8s ease-in-out infinite;
        }
        .animate-float-2 {
          animation: float-spark-2 10s ease-in-out infinite;
        }
      `}</style>
    </div>
  );
