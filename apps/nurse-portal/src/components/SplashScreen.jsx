...
export default function SplashScreen({ onDone }) {
  ...
  useEffect(() => {
    ...
    if (hour < 12) {
      setGreeting({
        en: "Good morning, Nurse",
        sw: "Asubuhi njema, Muuguzi"
      });
    } else if (hour < 16) {
      setGreeting({
        en: "Good afternoon, Nurse",
        sw: "Mchana mwema, Muuguzi"
      });
    } else {
      setGreeting({
        en: "Good evening, Nurse",
        sw: "Habari ya jioni, Muuguzi"
      });
    }

    const t1 = setTimeout(() => setPhase('show'), 100);
    const t2 = setTimeout(() => setPhase('exit'), 2200); // Faster duration for active clinical staff
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div 
            ...
            style={{ 
              animationDelay: '0.4s',
              background: 'radial-gradient(circle, #FFFFFF 0%, #0F4C81 60%, rgba(27, 107, 90, 0) 100%)',
