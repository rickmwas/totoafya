          ...
          {children.map(c => (
            <button key={c.id} onClick={() => setSelectedChild(c)}
              className={cn('px-4 py-2 rounded-full text-[13px] font-semibold border-2 transition-all',
                selectedChild?.id === c.id ? 'bg-[#0F4C81] text-white border-[#0F4C81]' : 'bg-white border-[#E5E5E5] text-[#666666]')}>
              {c.full_name}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1.5">
          ...
          <input type="date" value={givenDate} onChange={e => setGivenDate(e.target.value)}
            className="h-12 px-4 bg-white border border-[#E5E5E5] rounded-[14px] text-[14px] font-medium outline-none focus:border-[#0F4C81]" />
        </div>
        <div className="flex flex-col gap-1.5">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  given ? 'bg-[#0F4C81]/5 border-[#0F4C81]/30 opacity-60 cursor-default' :
                  isSelected ? 'bg-[#0F4C81]/8 border-[#0F4C81]' :
              ...
              >
                <div className={cn('w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 border-2',
                  given ? 'bg-[#0F4C81] border-[#0F4C81]' :
                  isSelected ? 'bg-[#0F4C81] border-[#0F4C81]' : 'border-[#D0D0D0]')}>
                  {(given || isSelected) && <Check size={12} className="text-white" strokeWidth={3} />}
                </div>
                <div className="flex-1">
                  <p className={cn('text-[13px] font-bold', given ? 'text-[#0F4C81]' : 'text-[#0A0A0A]')}>{v.name}</p>
                  <p className="text-[10px] text-[#A0A0A0]">Week {v.age_weeks}</p>
                </div>
                {given && <span className="text-[10px] font-bold text-[#0F4C81] bg-[#0F4C81]/10 px-2 py-0.5 rounded-full">Given</span>}
              </button>
            );
          })}
        </div>
