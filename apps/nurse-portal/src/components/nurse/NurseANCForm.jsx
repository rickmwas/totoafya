...
const Input = ({ value, onChange, type = 'text', placeholder }) => (
  <input
    ...
    className="h-12 px-4 bg-white border border-[#E5E5E5] rounded-[14px] text-[14px] font-medium text-[#0A0A0A] placeholder:text-[#A0A0A0] outline-none focus:border-[#0F4C81] transition-colors"
  />
);
    ...
    <div className="bg-white rounded-[24px] border border-[#E5E5E5] p-5 flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Urine Protein">
          <select value={form.urine_protein} onChange={e => setF('urine_protein', e.target.value)}
            className="h-12 px-4 bg-white border border-[#E5E5E5] rounded-[14px] text-[14px] font-medium text-[#0A0A0A] outline-none focus:border-[#0F4C81]">
          ...
          {[{ key: 'ttv_given', label: 'TTV' }, { key: 'ifas_given', label: 'IFAs' }, { key: 'llin_given', label: 'LLIN' }].map(({ key, label }) => (
            <button key={key} onClick={() => setF(key, !form[key])}
              className={cn('px-4 py-2.5 rounded-full text-[13px] font-semibold border-2 transition-all active:scale-[0.96]',
                form[key] ? 'bg-[#0F4C81] text-white border-[#0F4C81]' : 'bg-white border-[#E5E5E5] text-[#666666]')}>
              {label}
            </button>
          ))}
        </div>
      ...
      <Field label="Notes">
        <textarea value={form.notes} onChange={e => setF('notes', e.target.value)} placeholder="Additional clinical notes..."
          rows={3}
          className="px-4 py-3 bg-white border border-[#E5E5E5] rounded-[14px] text-[14px] font-medium text-[#0A0A0A] placeholder:text-[#A0A0A0] outline-none focus:border-[#0F4C81] transition-colors resize-none" />
      </Field>
      ...
      <button
        ...
        className={cn(
          ...
          saved ? 'bg-[#0F4C81] text-white' : 'bg-[#0F4C81] text-white shadow-green-glow disabled:opacity-50'
        )}
      >
