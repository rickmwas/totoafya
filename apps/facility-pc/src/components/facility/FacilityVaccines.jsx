...
const STATUS_STYLE = {
  given:     'bg-[#2E5B47]/10 text-[#2E5B47]',
  scheduled: 'bg-[#2E5B47]/10 text-[#2E5B47]',
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        {Object.entries(counts).map(([status, count]) => (
          <button key={status} onClick={() => setFilterStatus(filterStatus === status ? 'all' : status)}
            className={`px-4 py-2 rounded-full text-[12px] font-bold border-2 transition-all ${filterStatus === status ? 'border-[#2E5B47] bg-[#2E5B47]/5' : 'border-transparent bg-white'}`}>
